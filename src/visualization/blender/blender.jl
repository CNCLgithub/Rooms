...
function lights(r::Room)
    ...
    vec(map(light ∘ transform, Tuple.(cis)))
end
...
function camera(r::Room)
    ...
    x = sum(first.(pos)) / length(pos) + 0.5
...
@with_kw struct Blender <: RenderMethod
    blender::String="blender"
    script::String=default_script
    template=default_template
    mode::String="none"
    resolution::Tuple{Int, Int} = (400, 400)
    threads=Sys.CPU_THREADS
    color_map::Dict{Tile, RGB{Float32}} = Dict(
        floor_tile => RGB{Float32}(0, 0, 0),
        wall_tile => RGB{Float32}(0, 0, 1),
        obstacle_tile => RGB{Float32}(1, 0, 0)
    )
    path_color::RGB{Float32} = RGB{Float32}(0, 1, 0)
end

function render(m::Blender, r::Room, out::String;
                p::Vector{Int64} = Int64[])

    isdir(out) || mkpath(out)
    scene_out = joinpath(out, "scene.json")

    scene = translate(r, p) |> json
    open(scene_out, "w") do f
        write(f, scene)
    end
    # TODO: break into several lines
    cmd = `$(m.blender) --verbose 2 -noaudio --background  $(m.template) -P $(m.script) -t $(m.threads) --  --scene $(scene_out) --mode $(m.mode) --out $(out)  --resolution $(m.resolution)`
    run(cmd)
end
